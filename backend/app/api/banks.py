...
async def resync_core(db, slug: str, conn, date_from: str | None = None) -> dict | None:
    ...
    # авто-распределение по правилам + сопоставление контрагентов (новых создаём по имени
    # из выписки, чтобы операции не оставались без контрагента)
    ...
    new_names = sorted({(r.get("counterparty") or "").strip() for r in rows
                        if (r.get("counterparty") or "").strip()
                        and (r.get("counterparty") or "").strip().lower() not in parties})
    for name in new_names:
        cp = Counterparty(company_id=company_id, name=name)
        db.add(cp)
        parties[name.lower()] = cp
    if new_names:
        await db.flush()  # получить id вновь созданных контрагентов
    for r in rows:
